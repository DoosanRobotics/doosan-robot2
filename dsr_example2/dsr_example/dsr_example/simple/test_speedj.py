...
def main(args=None):
        ...
        try:
                from DSR_ROBOT2 import print_ext_result, movej, movejx, movesj, movesx, movel, movec, move_periodic, move_spiral, moveb, set_velx, set_accx, set_robot_mode, servoj, set_safety_mode, speedj
        ...
        while rclpy.ok():
                ...
                # speedj(vel=[0.0,0.0,0.0,0.0,40.0,0.0], acc=[60.0]*6,time=1.5)
                # time.sleep(3.0)
                # speedj(vel=[0.0,0.0,0.0,0.0,-40.0,0.0], acc=[60.0]*6,time=1.5)
                # time.sleep(3.0)
                ...
                for i in range(20):
                        speedj(vel=[0,0,0,0,i*1.0,0], acc=[30,30,30,30,30,30],time=0.2)
                        time.sleep(0.2)
                # movej(p1, vel=100, acc=100)
                time.sleep(1.5)
                for i in range(20):
                ...
                time.sleep(1.0)                
                # for i in range(20):
                #         speedj(vel=[i*1.0]*6, acc=[30.0]*6,time=0.2)
                #         time.sleep(0.2)
                # # movej(p1, vel=100, acc=100)
                # time.sleep(1.5)
                # for i in range(20):
