...
def main(args=None):
        ...
        try:
                from DSR_ROBOT2 import print_ext_result, movej, movejx, movesj, movesx, movel, movec, move_periodic, move_spiral, moveb, set_velx, set_accx, set_robot_mode, servoj, set_safety_mode, speedj, speedl
        ...
        time.sleep(2)
        servoj(posj(0,0,90,0,90,0), vel=[100.0]*6, acc=[100.0]*6, time=2.0)
        time.sleep(2)
        i = 0
        while i < 1:
                i += 1
                print("Moving forward...")
                speedl(vel=[0,0,10,0,0,0], acc=[100.0, 50.0], time=1.0)
                time.sleep(2.5)
                ...
                print("Gradual speed increase...")
                for i in range(10):
                        speedl(vel=[0,i,0,0,0,0], acc=[100.0, 50.0], time=0.5)
                        time.sleep(0.6)
                print("Gradual speed decrease...")
                for i in range(10):
                        speedl(vel=[0,-i,0,0,0,0], acc=[100.0, 50.0], time=0.5)
                        time.sleep(0.6)                
                # Stop motion
                print("Stopping...")
                speedl(vel=[0.0]*6, acc=[100.0, 50.0], time=0.5)
                time.sleep(1.0)
