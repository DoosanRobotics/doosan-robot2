...
class ServoCLI(Node):
    def __init__(self, node_name='test_servoj'):
        super().__init__(node_name)
        # replace 'dsr01' with your actual robot namespace if different
        topic = 'dsr01/servoj_stream'
        self._pub = self.create_publisher(ServojStream, topic, 10)
        self.get_logger().info(f'Publishing to {topic}')

    def send_servoj(self, pos, vel, acc, tm):
        msg = ServojStream()
        msg.pos = pos
        ...
        self.get_logger().info(f'→ servoj pos={pos} time={tm}s')

def parse_args():
    p = argparse.ArgumentParser(
        description='Publish one ServojStream message to the robot.')
    p.add_argument(
