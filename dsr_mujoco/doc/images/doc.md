...
Scene files are located in `dsr_description2/mujoco_models/<model_name>/` or `dsr_description2/mujoco_models/demo/` directory. In these files, you can add or modify elements such as floor, lighting, and objects.

### Compiler Tag

The `<compiler>` tag plays a crucial role in defining simulation settings, such as calculation methods and the `meshdir` attribute (which specifies the relative or absolute path to asset files like STL meshes).

Example:
```xml
<compiler angle="radian" meshdir="../" />
```
**Note**: For the robot model to load successfully, `meshdir` should point to `mujoco_models` directory.
